...
func TestGate_OpenNoReason(t *testing.T) {
	db := paDB(t)
	svc := NewService(NewRepository(db), &mockPAAlerter{})
	res, err := svc.SetFlag(context.Background(), padminActor(), SetFlagInput{Key: "ui.new_dashboard_beta", Enabled: true})
...
func TestGate_GuardedNeedsReason(t *testing.T) {
	...
	svc := NewService(NewRepository(db), &mockPAAlerter{})
	a := padminActor()
	if _, err := svc.SetFlag(context.Background(), a, SetFlagInput{Key: "connector.host_telemetry", Enabled: true}); status(err) != 400 {
		t.Fatalf("guarded flag must require a reason, got %v", err)
	}
	if _, err := svc.SetFlag(context.Background(), a, SetFlagInput{Key: "connector.host_telemetry", Enabled: true, Reason: "enable for pilot"}); err != nil {
...
func TestGate_ProtectedWeakenNeedsFourEyes(t *testing.T) {
	...
	// ai.egress_restricted secure=ON; setting it OFF is a weakening. No approver → four-eyes 403.
	_, err := svc.SetFlag(ctx, a, SetFlagInput{Key: "ai.egress_restricted", Enabled: false, Reason: "temp"})
	if status(err) != 403 {
		t.Fatalf("weakening a protected flag without a distinct approver must be 403, got %v", err)
	}
	// Same-user "approver" is not four-eyes.
	if _, err := svc.SetFlag(ctx, a, SetFlagInput{Key: "ai.egress_restricted", Enabled: false, Reason: "temp", ApprovedBy: &a.UserID}); status(err) != 403 {
	...
	approver := uuid.New()
	res, err := svc.SetFlag(ctx, a, SetFlagInput{Key: "ai.egress_restricted", Enabled: false, Reason: "incident triage", ApprovedBy: &approver})
...
func TestGate_ProtectedTowardSecureIsGuarded(t *testing.T) {
	...
	svc := NewService(NewRepository(db), &mockPAAlerter{})
	a := padminActor()
	// Enabling ai.egress_restricted (its secure state) is toward-secure → guarded (reason, no four-eyes).
	if _, err := svc.SetFlag(context.Background(), a, SetFlagInput{Key: "ai.egress_restricted", Enabled: true, Reason: "restore"}); err != nil {
...
func TestGate_RollbackReRunsGate(t *testing.T) {
	...
	if err := db.WithSystem(ctx, func(ctx context.Context, tx pgx.Tx) error {
		return tx.QueryRow(ctx, `SELECT id FROM platform_config_audit WHERE key='ai.egress_restricted' AND new_value->>'enabled'='false' ORDER BY created_at DESC LIMIT 1`).Scan(&weakenAuditID)
	}); err != nil {
		t.Fatalf("find audit: %v", err)
	}
	// Restore to secure.
	if _, err := svc.SetFlag(ctx, a, SetFlagInput{Key: "ai.egress_restricted", Scope: "global", Enabled: true, Reason: "restore"}); err != nil {
