...
// Password-reset confirmation — where an admin-issued reset link lands. The backend emails
// {resetBaseURL}/reset-password?token=<raw> (iam/password_reset.go); this screen drives the PUBLIC endpoint
// POST /auth/password-reset/confirm {token, new_password}. It is the consume-half of the admin "Reset password"
// action in Identity & access — without it that reset link dead-ended (F1 reconciliation gap). Mirrors the
// accept-invitation screen (the sibling token-consume page). Self-service "forgot password by email" is NOT wired
// (no public request endpoint — admin-issued only), so this page does not offer it.
...
function ResetForm() {
  ...
  async function submit() {
    ...
    setBusy(true);
    try {
      await apiPost<{ status: string }>("/auth/password-reset/confirm", { token, new_password: password });
      setDone(true);
    } catch (e) {
      if (e instanceof ApiError && (e.status === 401 || e.status === 400)) {
        setError("This reset link is invalid, expired, or already used. Ask your administrator to re-issue it.");
        ...
        >
          ...
          <div>
            ...
            <p className="mt-1 text-[13px]" style={{ color: "var(--c-ink-2)" }}>
              Your password has been reset. For your security, any existing sessions have been signed out — sign in
  ...
  return (
    <div className="w-full max-w-[380px]">
      ...
      <h2 className="mb-1.5 text-2xl font-bold tracking-tight">Choose a new password</h2>
      <p className="mb-8 text-sm" style={{ color: "var(--c-ink-2)" }}>
        Your administrator issued a password reset for your account. Set a new password below to continue.
      </p>
      ...
      >
        ...
        <p className="text-xs leading-relaxed" style={{ color: "var(--c-ink-3)" }}>
          This reset link is single-use and expires. Your password is hashed and never stored in plaintext; the reset
          revokes existing sessions and is recorded to the immutable audit trail.
        </p>
      </div>
    </div>
  );
