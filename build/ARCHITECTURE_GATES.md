...
**Verification:** 16 billing tests green (4 new slice-B adversarial + 12 slice-A); SECURITY-DEFINER-revoke guard, outbound-http guard, and schemacheck all green; all 80 migrations apply cleanly from zero on a fresh container DB (`ON_ERROR_STOP=1`). `go build ./...` + `go vet ./internal/billing/...` clean. **Awaiting reviewer landing round** (dedicated review of the account-rollup BOLA surface + suspension blast-radius, per the pre-code plan).
